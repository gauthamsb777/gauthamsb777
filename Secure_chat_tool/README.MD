...
<h2>Description</h2>
This script is a Python-based self-hosted secure chatroom application which uses client-server model to create a encrypted sessions. It provides a secure communication channel for multiple users, featuring invitation-based access, security questions, and real-time encrypted messaging.
<br />
...
- Color-coded messages to distinguish users 
- Public and private IP support for server hosting
- custom username support for users
...
- The application uses strong encryption methods, but when using in enterprise network, it should be regularly updated and audited.
- Hosts and users should be cautious about sharing invitation tokens and only use secure channel to share it.
- While the chat is end-to-end encrypted, metadata like frequency of message, timestamps etc maybe found through network admin logs
